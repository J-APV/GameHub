...
public class GameDiscounts extends Composite<VerticalLayout> {
    ...
    public GameDiscounts(ApiCall service) {
    ...
    private Tabs setTabSheetSampleData() {
        ...
        Tab current;
        Tabs tabs = new Tabs(
                current = new Tab(homeLink),
                new Tab(storeLink),
                new Tab(forumsLink)  // Add the RouterLink to the Tabs
        );
    ...
    private HorizontalLayout signupButton() {
        Button signupButton = new Button("Sign up");
        //signupButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        signupButton.setAutofocus(true);
        return new HorizontalLayout(signupButton);
    }
    ...
    private void updateList(String filter) {
        ...
        for (int i = 0; i < filteredDeals.size(); i++) {
            ...
            Card item = new Card(anchor, thumbImage, deal.getTitle(), salePrice, regularPrice);
